...
{
    conn.Open();
    using (var cmd = new NpgsqlCommand("SELECT \"SportId\", COUNT(*) FROM \"SportConcepts\" GROUP BY \"SportId\"", conn))
    {
        using (var reader = cmd.ExecuteReader())
        {
            Console.WriteLine("SportId | Count");
            Console.WriteLine("--------|------");
            while (reader.Read())
            {
                Console.WriteLine($"{reader.GetInt32(0)} | {reader.GetInt64(1)}");
            }
        }
    }
}
