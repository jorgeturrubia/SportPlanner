...
- [x] Task: Backend - Implement `MarketplaceService` with filtering (Author, Rating, Category).
- [x] Task: Backend - Implement `ItineraryService` for "Download/Link" logic (creating UserItineraries references).
- [x] Task: Backend - Implement `RatingService` with background process to update `AverageRating` in the main table.
- [x] Task: Backend - Create `MarketplaceController` and update `ItinerariesController` for shadowing logic.
...
- [x] Task: Frontend - Define DTOs and Interfaces for Itineraries (with `IsSystem` and `Author` metadata).
- [x] Task: Frontend - Create `MarketplaceService` to interact with the new backend endpoints.
- [x] Task: Frontend - Implement `MarketplaceComponent` (Sidebar entry "Marketplace").
- [x] Task: Frontend - Build Marketplace Grid with filters (Rating, Author) and "Download" action.
...
- [x] Task: Frontend - Create `ItineraryListComponent` ("My Itineraries").
- [x] Task: Frontend - Implement visual badges for "System" vs "User" itineraries.
...
- [x] Task: Frontend - Implement the Star Rating component in the Marketplace.
