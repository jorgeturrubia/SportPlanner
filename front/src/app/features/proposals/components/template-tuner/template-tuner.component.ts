...
@Component({
    ...
    template: `
    <div class="bg-dark-surface border-b border-dark-border py-1 px-3 flex items-center justify-between shadow-2xl z-20">
      
      <!-- Left: Template Selection -->
      <div class="flex items-center gap-3 w-full">
        <div class="flex flex-col">
            <span class="text-secondary tracking-[0.2em] uppercase text-[9px] font-black">Metodología SportPlanner</span>
        ...
        <div class="flex flex-col">
            <span class="text-gray-500 uppercase text-[9px] font-bold tracking-widest">Nivel Fase</span>
            <span class="font-black text-xs uppercase tracking-tight text-secondary">
                {{ getActiveLevelLabel() }}
            </span>
        </div>
        ...
        <!-- Create Concept Button -->
         <button type="button" (click)="onCreateClick()" 
            class="flex items-center gap-2 bg-primary/10 hover:bg-primary/20 text-primary px-2 py-1 rounded font-bold uppercase text-[10px] tracking-wider transition-colors border border-primary/30">
